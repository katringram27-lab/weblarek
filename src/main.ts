...
const events = new EventEmitter(); // создаем объект брокера событий events

        
        //const apiProducts = { // создали объект apiProducts
            //items: [
        //{
...
console.log("Обновленные товары в корзине после добавления нового товара:", cartModel.getItems());

cartModel.removeItem(apiProducts.items[0].id); // удаляем товар по его ID из корзины
console.log("Обновленные товары в корзине после удаления товара:", cartModel.getItems());
...
console.log("Товары в корзине после добавления нескольких товаров:", cartModel.getItems());
...
console.log(`Содержит ли корзина товар с id ${apiProducts.items[1]?.id}:`, exists);
...
const apiBuyer = { // пример данных покупателя
    //payment: 'online',
...
buyerModel.setData({ // устанавливаем новые данные для покупателя
...
(async () => {
    try {
        const products: IProduct[] = await appApi.getProducts();
