...
export class CartModel {
  // создали класс CartModel

  protected items: IProduct[]; // массив, хранящий товары в корзине

  constructor(protected events: IEvents) {
    //передаем в корзину менеджер событий events
    this.items = []; // список товаров сбрасываем в пустой массив
  }

  getItems(): IProduct[] {
    // возвращаем текущий массив всех товаров, добавленных в корзину
    return this.items;
  }

  addItem(item: IProduct): void {
    // добавляем новый товар в конец массива items
    this.items.push(item);
  }

  removeItem(id: string): void {
    // фильтруем массив, оставляя только те товары, у которых id не совпадает с переданнымю Товар с нужным id удаляется
  ...
  contains(id: string): boolean {
    // проверяем,есть ли в корзине товар с указанным id. Метод some возвращает true, если хотя бы один товар найден, иначе - false
